...
if __name__ == '__main__':
	...
	if len(sys.argv) != 6:
		sys.exit('usage: stats_edges.py <snapshots_init,snapshots_end> <input_graph_file_path> <factor> <T/N>')
	...
	mod = 'snapshot'#'trg'#'snapshot' 'trg'
	split_mod = sys.argv[4]#'T' # 'N' # 'TS'
	...
	output_aggre_file_path = input_graph_file_path.split('/')[-1].split('.')[-2] + '_' + split_mod + '_agg.tsv'
	# output_temp_file_path = input_graph_file_path.split('/')[-1].split('.')[-2] + '_' + split_mod + '.tsv'

	fOut_aggre = open(output_aggre_file_path, 'w')
	# fOut_temp = open(output_temp_file_path, 'w')

	EDGE_WEI_DICT = defaultdict(float)

	decay = float(sys.argv[3])

	for idx in range(init, end+1):

		cur_pow = np.power(decay, (end-idx))
		...
		for line in lines:
			...
			weight = float(wei_str)

			EDGE_WEI_DICT[edge] += weight * cur_pow


	for edge in EDGE_WEI_DICT:
		fOut_aggre.write( str(edge[0]) + '\t' + str(edge[1]) + '\t' + str( round(EDGE_WEI_DICT[edge], 6) ) + '\n' )
